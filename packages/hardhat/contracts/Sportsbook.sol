...
contract Sportsbook {
  ...
  function deleteChallenge(uint256 _challengeId) public {
    ...
    (bool success, ) = payable(matchChallenges[_challengeId].team1).call{value: matchChallenges[_challengeId].bet}("");
    require(success, "Sportsbook: Transfer to team1 failed.");
    if (matchChallenges[_challengeId].state == MatchState.ACCEPTED) {
      (bool success2, ) = payable(matchChallenges[_challengeId].team2).call{value: matchChallenges[_challengeId].bet}(
        ""
      );
  ...
  function viewMatchChallenge(
    uint256 _id
  ) public view returns (address team1, address team2, MatchState state, uint256 bet, address referee) {
